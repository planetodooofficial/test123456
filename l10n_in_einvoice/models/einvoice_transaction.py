...
class L10nInInvoiceTransaction(models.Model):
    ...
    def get_credit_day(self, move):
        if move:
            payment_term_line_id = move.line_ids.filtered(lambda t: t.account_id.user_type_id.type == 'receivable')
    ...
    def _compute_generate_request_json(self):
        for transaction in self.filtered(lambda t: t.move_id.state == 'posted'):
            ...
            generate_request_json = self.env['ir.ui.view']._render_template("l10n_in_einvoice.l10n_in_invoice_request_payload_json", values)
